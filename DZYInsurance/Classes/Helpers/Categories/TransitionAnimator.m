...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext {
    ...
    if (!self.layerView) {
        UIView *layerV = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    ...
    CGFloat viewW = SCREEN_WIDTH - viewX;
    if (self.animationType == AnimationTypeDismiss) {
        UIView *snap = [fromView snapshotViewAfterScreenUpdates:YES];
        fromView.frame = CGRectMake(SCREEN_WIDTH, 0, viewW, SCREEN_HEIGHT);
        
        [[transitionContext containerView] addSubview:snap];
        snap.frame = CGRectMake(viewX, 0, viewW, SCREEN_HEIGHT);
        [UIView animateWithDuration:[self transitionDuration:transitionContext] animations:^{
            self.layerView.alpha = 0.0f;
            snap.frame = CGRectMake(SCREEN_WIDTH, 0, viewW, SCREEN_HEIGHT);
        ...
        [UIView animateWithDuration:[self transitionDuration:transitionContext] animations:^{
            ...
            snap.frame = CGRectMake(viewX, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
        } completion:^(BOOL finished) {
